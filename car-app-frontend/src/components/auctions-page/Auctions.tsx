				...
				<SortRow flex={1}>
					<Col flex={1}>
						<label>
							...
							<Select
								onSelect={(value) =>
									applyFilter(Filter.TRANSMISSION, value.toString())
								}
					...
					<Col flex={1}>
						<label>
							...
							<Select
								onSelect={(value) =>
									applyFilter(Filter.BODY_STYLE, value.toString())
								}
				...
				<NewsCol flex={1}>
					...
					<label>
						...
						<DatePicker
							...
							className="full-width"
							defaultValue={moment(
								getParamFromUrl(Filter.TIME_RANGE, '31-12-2021').replace(
									'-',
									'/'
								),
								dateFormat
					...
					<label>
						...
						<Slider
							...
							range
							defaultValue={[
								Number(getParamFromUrl(Filter.MIN_PRICE, '0')),
								Number(getParamFromUrl(Filter.MAX_PRICE, '100000')),
							]}
							min={0}
							max={100000}
							onAfterChange={(values) => {
								applyPriceRangeFilter(values[0], values[1]);
							}}
						/>
					</label>
					<label>
					...
					<PrimaryButton className="full-width" icon={<ClearOutlined />}>
						Clear filters
					</PrimaryButton>
				</NewsCol>
			</Row>
